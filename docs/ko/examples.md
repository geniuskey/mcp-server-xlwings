...
> "이 사업수지 분석 엑셀 파일에서 주요 재무 지표를 추출해줘"

5개 시트, 병합 헤더, 수식이 포함된 복잡한 Excel 파일을 처리하는 단계별 워크플로우:
...
워크북 이름, 5개 시트의 크기와 사용 범위를 확인합니다.
...
한 번의 호출로 5개 시트 모두의 요약을 확인합니다: 헤더, 병합 셀, 영역 정보.

**3단계 — 헤더 구조 파악:**

```
read_data(sheet="수지분석", cell_range="A1:Z5", merge_info=true)
```

"건축 계획"이 6-19행에 걸쳐 병합된 것과 같은 다단계 병합 헤더를 확인합니다.

**4단계 — 계산식 찾기:**

```
get_formulas(sheet="수지분석", cell_range="A1:Z100", values_too=true)
```

모든 수식 셀과 계산된 값을 식별합니다.

**5단계 — 서식으로 섹션 구분:**

```
get_cell_styles(sheet="수지분석", cell_range="A1:A50", properties=["bold", "bg_color"])
```
...
```
read_data(sheet="수지분석", cell_range="B6:U36", header_row=2)
```

2행을 헤더로 지정하여 특정 데이터 테이블을 읽습니다.
